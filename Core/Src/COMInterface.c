		...
		if (current_menu == MAIN_MENU) {
			if (strcmp("clear", command) == 0) {
				...
				*dest = '\0';
				...
				f_rename(src, dest);
			} else if (memcmp(command, "msc", 3) == 0) {
				if (strlen(command) != 4) {
			...
			} else if (memcmp(command, "bootmode", 8) == 0) {
			...
			else if (memcmp(command, "dms", 3) == 0) {
				char* time = command + 4;
			...
			} else if (memcmp(command, "hostnamectl", 11) == 0) {
				char* hostname = command + sizeof("hostnamectl");
				...
				if (*folder_name == '\0') {
					fputs("Invalid format. Expects `mkdir {name}`\n", stderr);
				} else {
					f_mkdir(folder_name);
				}
			}
				...
				if (*file == '\0') {
					fputs("Invalid format. Expects `cat {file-name}`\n", stderr);
				} else {
					FIL f;
				...
				NVIC_SystemReset();
			} else if (memcmp("pwd", command, 3) == 0) {
			...
			else if (memcmp("rmdir", command, 5) == 0) {
			...
			if (strcmp(command, "exit") == 0) {
				...
				printf("%d\n", (int) status);
			}  else if (strcmp(command, "pfirm") == 0) {
