...
void RP_DISPLAY_CLEAR() {
	systemCallData.command = CLEAR_LCD;
	systemCallData.args = (void*) 0;
...
// file controls
void RP_OPENDIR(const char* folder) {
	systemCallData.args = folder;
	systemCallData.command = OPENDIR;

	__asm volatile("SVC #0");
}

void RP_CLOSEDIR() {
	systemCallData.command = CLOSEDIR;

	__asm volatile("SVC #0");
}

void RP_NEXTFILE(const char filename[256], uint8_t* attributes) {
	struct {
		const char** filename;
		uint8_t attributes;
	} args;
	...
	systemCallData.command = NEXT_FILE;
...
void RP_PRINT_TEXT(char* text, uint8_t* page, uint8_t* col) {
	...
	systemCallData.args = (void*) &args;
	systemCallData.command = 0x0200;

	system_call(0x200, (void*)&args);
}

void RP_DRAW_ICON(const uint8_t* bitmap, const uint8_t width_bits, const uint8_t height_bytes, uint8_t* page, uint8_t* col) {
	struct {
		uint8_t* bitmap;
		uint8_t width_bits;
	...
	args.bitmap = bitmap;
	args.width_bits = width_bits;
	args.height_bytes = height_bytes;
	args.page = page;
	args.col = col;

	systemCallData.args = (void*) &args;
	systemCallData.command = 0x0201;


	system_call(0x0201, (void*)&args);
}

uint8_t* RP_GET_LCD_BUFFER() {
	systemCallData.command = GET_LCD_BUFFER;
